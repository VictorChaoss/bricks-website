...
// ── Pollinations image generation (Flux, free, no key needed) ─────────────────
// Retries across two models on 429. Always returns a base64 data URI.
async function generateImage(prompt) {
  ...
  for (let i = 0; i < models.length; i++) {
    ...
    const url   = `https://image.pollinations.ai/prompt/${encoded}?width=512&height=512&model=${model}&nologo=true&seed=${seed}`;

    console.log(`[Image] Trying Pollinations model=${model}`);
    ...
    if (res.ok) {
      ...
      if (buf.length < 500) {
        console.warn(`[Image] ${model} returned near-empty image (${buf.length} bytes), retrying`);
        continue;
      }
      console.log(`[Image] Success with model=${model}, size=${buf.length}`);
      return `data:${ct};base64,${buf.toString('base64')}`;
    }

    if (res.status === 429) {
      console.warn(`[Image] ${model} rate limited (429)`);
...
export default async function handler(req, res) {
  ...
  try {
    // ── Step 1: Vision — GPT-4o-mini reads the face ───────────────────────────
    console.log('[Vision] Sending image to GPT-4o-mini');
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            {
              type: "text",
              text: "Describe this person's physical appearance for an AI image generator. Include: hair color and style, skin tone, eye color if visible, face shape, and any notable clothing. 1-2 sentences, specific and factual."
            },
    ...
    // ── Step 2: Generate LEGO image via Pollinations (Flux) ───────────────────
